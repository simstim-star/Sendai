...
int Sendai_run() {
	...
	bool load_success = SendaiGLTF_load("BoxVertexColors.gltf", &engine.renderer.model);
...
void init_window(Sendai* engine) {
	...
	engine->hwnd = CreateWindow(wc.lpszClassName, engine->title, WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT,
										 rect.right - rect.left, rect.bottom - rect.top, NULL, NULL, engine->hinstance, NULL);
...
LRESULT CALLBACK window_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
	...
	if (SendaiGui_handle_event(hwnd, message, wparam, lparam)) return 0;
